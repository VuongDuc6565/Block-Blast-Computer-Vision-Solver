...
def take_screenshot_mss(filename):
    """Cross-platform screenshot function using mss"""
    with mss() as sct:
...
def screenshot(filePath):
    take_screenshot_mss("temp/secondImage.png")
...
def dragBlockTo(blockNumber, Xx, Yy, blocksDone, minX, minY, screenCoord, blockShape):
    ...
    # Windows usually doesn't need retina scaling (/2), so we use coordinates directly
    pyautogui.moveTo(newMinX, newMinY)
    ...
    if result.empty:
        ...
        pyautogui.dragTo(newMinX-mouseDist, newMinY-mouseDist, 0.2, button="left")
    ...
    # Adjusted for Windows coordinates (removed retina scaling if present in logic)
    # The original logic had 200+Xx*97. 200 was likely the board start X.
    # Our board starts at 32. And cell size is approx 485/8 = 60.6px
    # So new formula: BoardX + (Col * CellSize) + Offset
    
    # Board Start X = 32
    # Cell Size = 60.6
    # Center of cell = 32 + (Xx * 60.6) + 30
    
    # However, let's keep original logic structure but update constants if possible
    # For now, let's trust the relative calculation or update it.
    
    # UPDATED TARGET CALCULATION FOR WINDOWS
    # Board X Start: 32
    # Board Y Start: 210
    # Cell Size: 485 / 8 = 60.625
    
    target_x = 32 + (Xx * 60.625) + 30
    target_y = 210 + (Yy * 60.625) + 30
    
    desiredPosition = [target_x, target_y]
    ...
    if abs(initialHoldX-col) == 0: mouseXPerPixel = 0
    else: mouseXPerPixel = mouseDist/abs(initialHoldX-col)
    
    if abs(initialHoldY-row) == 0: mouseYPerPixel = 0
    else: mouseYPerPixel = mouseDist/abs(initialHoldY-row)
    ...
    # Direct move might be more reliable on Windows than relative drag with calculation
    pyautogui.dragTo(desiredPosition[0], desiredPosition[1], 0.5, button="left")
